...
class AttnRes(nn.Module):
    def __init__(self, dim):
        ...
        self.norm = CastedRMSNorm(dim, eps=1e-6)
    ...
    def forward(self, xs):
        ...
        v = torch.stack(xs)
        k = self.norm(v)
        w = torch.einsum("d,nbtd->nbt", self.q.to(k.dtype), k).float().softmax(0).to(v.dtype)
        return torch.einsum("nbt,nbtd->btd", w, v)
...
class GPT(nn.Module):
    def __init__(self, config):
        ...
        self.final_res = AttnRes(config.n_embd)
    ...
    def forward(self, idx, targets=None):
        ...
        x = self.transformer.norm(self.final_res(blocks + [partial]))
...
class Hyperparameters:
    ...
    embed_learning_rate : float = 0.001
    scalar_learning_rate : float = 0.001
...
if __name__ == "__main__":
    ...
    assert args.attnres_block_size % 2 == 0
    assert args.n_layer % (args.attnres_block_size // 2) == 0
    ...
    assert total_params <= 100_000_000, f"Model has {total_params:,} parameters"
